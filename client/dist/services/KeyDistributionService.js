...
exports.KeyDistributionService = void 0;
class KeyDistributionService {
    constructor(signalProtocol, groupProtocol, apiUrl, authToken) {
     ...
     */
    async distributeGroupKeys(groupId, distributionMessage) {
        ...
        const distributions = members.map(memberId => this.sendKeyDistributionDM(memberId, groupId, serializedDistribution));
        await Promise.all(distributions);
    }
    /**
     * Send sender key distribution via encrypted DM
     */
    async sendKeyDistributionDM(recipientId, groupId, senderKeyDistribution) {
        ...
        const message = {
            ...
            groupId,
            senderKeyDistribution: Array.from(senderKeyDistribution),
            timestamp: Date.now()
        };
        // Encrypt the message
     ...
     */
    async processKeyDistributionMessage(senderId, encryptedMessage) {
        // Decrypt the message
        // Assuming PreKey message type for key distribution
        const plaintext = await this.signalProtocol.decryptMessage(senderId, 1, // deviceId
        encryptedMessage, 2 // CiphertextMessageType.PreKey
        );
        ...
        await this.groupProtocol.processSenderKeyDistribution(message.groupId, senderId, 1, // Device ID - we're assuming single device for now
        new Uint8Array(message.senderKeyDistribution));
    }
    /**
     * Request sender keys from all group members
     */
    async requestGroupKeys(groupId) {
        const members = await this.fetchGroupMembers(groupId);
        // Send key request to each member
        const requests = members.map(memberId => this.sendKeyRequest(memberId, groupId));
        await Promise.all(requests);
    }
    /**
     * Send a request for sender keys
     */
    async sendKeyRequest(recipientId, groupId) {
     ...
     */
    async rotateAndDistributeKeys(groupId) {
        // Rotate our keys
    ...
    async establishSession(userId) {
        ...
        await this.signalProtocol.processPreKeyBundle(userId, 1, // deviceId - assuming single device
        {
     ...
     */
    async sendEncryptedDM(recipientId, ciphertext) {
    ...
    async handleNewMember(groupId, newMemberId) {
        ...
        await this.sendKeyDistributionDM(newMemberId, groupId, ourDistribution.serialize());
        // Request their keys
        await this.sendKeyRequest(newMemberId, groupId);
    }
}
exports.KeyDistributionService = KeyDistributionService;
//# sourceMappingURL=KeyDistributionService.js.map