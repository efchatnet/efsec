...
export class DMService {
  private config: DMServiceConfig;
   ...
   */
  async initiateDM(peerId: string): Promise<DMSpace> {
   ...
   */
  private async findExistingDM(peerId: string, token: string): Promise<DMSpace | null> {
   ...
   */
  private async establishE2ESession(peerId: string): Promise<void> {
    if (!this.signalManager) {
   ...
   */
  async getUserDMs(): Promise<DMSpace[]> {
    const token = this.config.getAuthToken();
    if (!token) {
   ...
   */
  async isDMSpace(spaceId: string): Promise<boolean> {
    const token = this.config.getAuthToken();
    if (!token) {
      return false;
    }
   ...
   */
  async enableE2EForSpace(spaceId: string): Promise<void> {
    const token = this.config.getAuthToken();
    if (!token) {
      throw new Error('Authentication required');
    }
