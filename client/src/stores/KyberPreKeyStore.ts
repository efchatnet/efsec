...
export class KyberPreKeyStoreImpl extends KyberPreKeyStore {
  ...
  private dbName = 'SignalKyberPreKeyStore';
  private storeName = 'kyberPreKeys';
  ...
  async saveKyberPreKey(keyId: number, record: KyberPreKeyRecord): Promise<void> {
  ...
  async getKyberPreKey(keyId: number): Promise<KyberPreKeyRecord> {
    // Try memory first
  ...
  async markKyberPreKeyUsed(keyId: number): Promise<void> {
    // Remove from memory
  ...
  async countKyberPreKeys(): Promise<number> {
    if (this.db) {
      return new Promise((resolve, reject) => {
        const transaction = this.db!.transaction([this.storeName], 'readonly');
        const store = transaction.objectStore(this.storeName);
