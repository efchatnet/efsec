...
export async function generateOneTimeKeys(count: number): Promise<KeyPair[]> {
  ...
  // If no keys found in existing requests, this indicates the OlmMachine hasn't generated
  // enough keys yet. This is normal - keys will be generated by the Matrix crypto library
  // when needed during the normal key upload process.
  if (keys.length === 0) {
    console.log('[Matrix] No pre-generated keys available. Keys will be generated during upload process.');

    // Return empty array - this will trigger key generation through normal Matrix flows
    // The OlmMachine will generate keys internally when uploadKeys() is called
    return [];
  }
